...
def _save_seg_nrrd(segmentationNode, labelmapNode, path):
    ...
    keys (Segment0_*, Segmentation_*) as custom NRRD attributes. Computes
    each segment's IJK bounding box from the labelmap — Slicer uses the
    Segment*_Extent field to find voxels for that segment, so a stub
    0 0 0 0 0 0 extent makes the segment appear empty in the GUI."""
    ...
    # Build label-volume array once and compute per-label IJK bounding boxes
    from vtk.util.numpy_support import vtk_to_numpy
    label_arr = vtk_to_numpy(image.GetPointData().GetScalars())
    dims = image.GetDimensions()                  # (i, j, k)
    label_arr = label_arr.reshape(dims[::-1])     # numpy is (k, j, i)

    def _extent_for_label(value):
        mask = (label_arr == value)
        if not mask.any():
            return "0 0 0 0 0 0"
        k_idx, j_idx, i_idx = np.where(mask)
        return f"{int(i_idx.min())} {int(i_idx.max())} " \
               f"{int(j_idx.min())} {int(j_idx.max())} " \
               f"{int(k_idx.min())} {int(k_idx.max())}"
    ...
    try:
        ...
        for idx, sid in enumerate(seg_ids):
            ...
            w.SetAttribute(f"Segment{idx}_LabelValue", str(lv))
            w.SetAttribute(f"Segment{idx}_Layer", "0")
            w.SetAttribute(f"Segment{idx}_Extent", _extent_for_label(lv))
